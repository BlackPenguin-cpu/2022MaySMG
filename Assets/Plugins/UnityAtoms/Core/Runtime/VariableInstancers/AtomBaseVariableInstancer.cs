...
namespace UnityAtoms
{
        ...
        where V : AtomBaseVariable<T>
    {
...
#if ATOMS_CUSTOM
        [ShowInInspector]
        public bool isRegisterAtContainer = false;
        [ShowInInspector, ShowIf("isRegisterAtContainer")]
        public string id;

        [ShowInInspector]
        public bool isMemoryCopy = true;

        [ShowInInspector]
        public T ShowedValue { get => _inMemoryCopy == null ? Base == null ? default(T) : Base.Value : _inMemoryCopy.Value; }

         /// <summary>
        /// Getter for retrieving the in memory runtime variable.
        /// </summary>
        public V Variable { get => isMemoryCopy ? _inMemoryCopy : _base; }
        ...
        public T Value
        {
            get => isMemoryCopy ? _inMemoryCopy.Value : _base.Value;
            set
            {
                if (isMemoryCopy) _inMemoryCopy.Value = value;
                else _base.Value = value;
            }
        }

        [ShowIf("isMemoryCopy")]
        public bool isInitalizeValue = false;

        [ShowIf("@isMemoryCopy && isInitalizeValue")]
...
#if ATOMS_CUSTOM
            if (isMemoryCopy && isInitalizeValue)
            {
                _inMemoryCopy.Value = initialValue;
            }

            if (isRegisterAtContainer)
            {
                VariableInstancerContainer.RegisterInstance(this);
            }
#endif
            ImplSpecificSetup();
        }

#if ATOMS_CUSTOM && ODIN_INSPECTOR
        [DisableInEditorMode, Button, ShowIf("isMemoryCopy")] 
        public void SetValue(T value)
        {
            if (isMemoryCopy)
            {
                _inMemoryCopy.Value = value;
            }
        }
#endif
    }
